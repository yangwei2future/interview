...
Kafka 每条消息（Record）由两部分组成：

| | 作用 | 序列化器 | 举例 |
|------|------|------|------|
| **key** | 决定消息发到哪个分区（路由坐标） | `key.serializer` | `"user_123"`、`1001L` |
| **value** | 消息的实际内容/载荷（payload） | `value.serializer` | `{"orderId":1001,"amount":99}` |

**分区路由公式：** `hash(key) % 分区数` → 确定发到哪个 Partition

```java
// key=null → 随机轮询分区（不关心顺序）
producer.send(new ProducerRecord<>("topic", null, message));

// key=指定 → 相同 key 进入同一分区（保证顺序）
producer.send(new ProducerRecord<>("topic", userId, orderData));
```

**为什么 key 和 value 各要指定序列化器？** 因为两者通常是不同类型：
- key 通常是 String/Long → StringSerializer / LongSerializer
- value 通常是 JSON/Avro/Protobuf → JsonSerializer / AvroSerializer
...
Kafka 单机百万级 TPS，靠四个设计：

**1. 顺序写磁盘**
消息只追加到 Partition 日志末尾，不修改不删除，顺序写比随机写快很多。

**2. 页缓存**
写消息先写内存（操作系统页缓存），异步刷盘。Consumer 大概率直接从内存读，不用碰磁盘。

**3. 零拷贝**
数据从磁盘发到网卡，不经过用户空间，减少 CPU 参与的拷贝次数，节省 CPU 资源。
面试一句话：零拷贝让数据传输绕过用户空间，减少拷贝次数，降低 CPU 开销。

**4. 批量发送 + 压缩**
```
Producer：攒够16KB或等5ms，打包一起发 → 减少网络请求次数
Consumer：poll() 一次拉一批             → 减少 poll 调用次数
压缩（lz4/snappy）：减少网络传输数据量
```

**面试答法：** Kafka 高性能主要靠顺序写、页缓存、零拷贝、批量压缩四点，说完这四个关键词基本满意。
