...
有些问题一条 SQL 搞不定，需要 Agent 多步推理：

**场景：**"上个月 GMV 增长最快的三个品类？"

```
Step 1: Agent 调用 search_indicator("GMV") → 返回公式和字段
Step 2: Agent 调用 get_table_schema("orders") → 返回表结构
Step 3: LLM 生成 SQL_1 → 查本月各品类 GMV
Step 4: LLM 生成 SQL_2 → 查上月各品类 GMV
        （Step 3 和 Step 4 无依赖，可并行执行）
Step 5: Agent 拿到两个结果 → 计算环比增长率 → 排序取 Top3
Step 6: 返回最终结果
```

**依赖关系自动解析**：Agent 构建子问题的 DAG（有向无环图），识别哪些步骤可并行、哪些必须串行。

**核心策略："宁可多步简单查询，不要一步复杂查询"**。窗口函数（`LAG() OVER()`）一条 SQL 也能搞定，但 NL2SQL 场景下 LLM 写复杂窗口函数的准确率远低于分步简单查询。
