...
### 为什么不能在 new 的时候直接判断代理，放入二级缓存？

技术上可以，但 Spring 不这么干，原因就一个：**懒加载，省开销。**

```
new 时就判断代理：
  每个 bean 都要遍历所有 Advisor、做切点匹配 → 100% 都要买单
  但 99% 的 bean 根本就没有循环依赖

三级缓存存工厂：
  只存一个函数引用（几字节），不执行
  只有真正发生循环依赖时（1%的概率），工厂才被触发
```

**结论：三级缓存就是懒加载策略。没有循环依赖，工厂 lambda 白注册但零开销。**
